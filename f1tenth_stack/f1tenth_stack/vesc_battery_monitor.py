    ...
    def vesc_state_callback(self, msg):
        ...
        if voltage < self.warning_threshold and voltage >= self.voltage_threshold:
            if not self.warning_issued:
                self.get_logger().warn(f"WARNING: Voltage {voltage:.2f}V is below warning threshold {self.warning_threshold}V")
        ...
        if voltage < self.voltage_threshold:
            ...
            if self.low_voltage_start_time is None:
                ...
                self.get_logger().warn(f'Low voltage detected: {voltage:.2f}V (threshold: {self.voltage_threshold}V)')
            else:
                ...
                if duration >= self.check_duration:
                    self.get_logger().error(f'Voltage critically low for {duration:.1f}s. Shutting down system...')
